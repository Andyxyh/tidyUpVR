...
let buttonCounter = 60;

let levelFrame = 3600;
let levelTime = levelFrame / 60;
let timeUsedRoom1, timeUsedRoom2, timeUsedRoom3;


function preload() {
	inventoryScissors = loadImage("images/ScissorsInventory.png");
	inventoryTshirt = loadImage("images/tshirtInventory.png");
	inventoryToothbrush = loadImage("images/bathroom/inventory/toothbrushInventory.png");
	inventoryChair = loadImage("images/chairInventory.png");
	inventoryTowel = loadImage("images/bathroom/inventory/towelInventory.png");
	inventoryToothpaste = loadImage("images/bathroom/inventory/toothpasteInventory.png");
	inventoryShampoo = loadImage("images/bathroom/inventory/shampooInventory.png");
	inventoryBooks = loadImage("images/booksInventory.png");

	inventoryBanana = loadImage("images/kitchen/inventory/bananaInventory.png");
	inventoryDishes = loadImage("images/kitchen/inventory/dishesInventory.png");
	inventoryPot = loadImage("images/kitchen/inventory/potInventory.png");
	inventorySoup = loadImage("images/kitchen/inventory/soupInventory.png");
	inventoryToaster = loadImage("images/kitchen/inventory/toasterInventory.png");
	inventoryPaper = loadImage("images/kitchen/inventory/paperInventory.png");

	instruction1 = loadImage("images/instruction1.png");
	instruction2 = loadImage("images/instruction2.png");
	instruction3 = loadImage("images/instruction3.png");
	levelCompleteImage = loadImage("images/levelComplete.png");
	gameCompleteImage = loadImage("images/gameComplete.png");
	ratingA = loadImage("images/A.png");
	ratingS = loadImage("images/S.png");
	ratingB = loadImage("images/B.png");

	note1 = loadImage("images/note1.png");
	note2 = loadImage("images/note2.png");
	note3 = loadImage("images/note3.png");
	gameCompleteRating = loadImage("images/gameCompleteRating.png");
}
...
function setup() {
	...
	world.setUserPosition(0, userHeight, 0);
	...
	cameraContainer = new Container3D({});

	inventoryBuffer = createGraphics(250, 250);
	inventoryTexture = world.createDynamicTextureFromCreateGraphics( inventoryBuffer );
	inventoryBuffer.clear();

	instructionBuffer = createGraphics(500, 500);
	instructionTexture = world.createDynamicTextureFromCreateGraphics( instructionBuffer );

	noteBuffer = createGraphics(300, 300);
	noteTexture = world.createDynamicTextureFromCreateGraphics( noteBuffer );

	levelCompleteBuffer = createGraphics(500, 500);
	levelCompleteTexture = world.createDynamicTextureFromCreateGraphics( levelCompleteBuffer );

	inventoryBoard = new Plane({
	...
	instructionBoard = new Plane({
		asset: instructionTexture,
		x: 0, y: 0, z: 0.2, 
	...
	levelCompleteBoard = new Plane({
		asset: levelCompleteTexture,
		x: 0, y: 0, z: 0.2, 
		width: 0.7, height: 0.7,
		rotationY: 0,
		dynamicTexture: true,
	...
	cameraContainer.addChild(inventoryBoard);
	cameraContainer.addChild(instructionBoard);
	cameraContainer.addChild(noteBoard);
	cameraContainer.addChild(levelCompleteBoard);
	cameraContainer.addChild(timer);
	cameraContainer.addChild(hourGlass);

	// instructionBuffer.clear();
	noteBuffer.background(255, 0, 0);
	// levelCompleteBuffer.background(128, 0, 129);

	world.camera.cursor.addChild(cameraContainer);
	...
	room1container.addChild(corkboard);
	...
	room1container.addChild(painting);
	...
	room1container.addChild(bookcase);
	...
	// Closet spot
	spots.push(new Spot(2.5, 1, -1, "room1"));

	//Bed spot
	spots.push(new Spot(-2, 0.55, -1, "room1"));

	//bookcase spot
	spots.push(new Spot(-1, 0.4, -2.5, "room1"));
	...
	scissors1 = new Model("scissors_gltf", inventoryScissors,    1, 0.01, 0,      2,      0.6, 0.2, 0.6,     0, 0, 0,     0, 0, 0,     0.045,     2.5, 2,     'room1');
	room1Items.push(scissors1);
	
	tShirt1 = new Model("tshirt_gltf", inventoryTshirt,    -2, 0.9, -2,     0.001,     0.8, 0.8, 0.5,     0, 0, 0,     0, -1.3, 0,     0.3,     2.5, -1,     'room1');
	room1Items.push(tShirt1);

	// chair1 = new Model("chair_gltf", inventoryChair,     -2, 0.5, 0,     0.01,     0.7, 0.7, 0.7,     0, 95, 0,     0, -0.5, -0.2,     0.5,     1.5, 1.7,      'room1');
	// room1Items.push(chair1);

	chair1 = new Model("chair_glb", inventoryChair,     -2, 0.5, 0,     1.7,     0.9, 0.9, 0.9,     0, 260, 0,     0.3, -0.5, 0.2,     0.5,     1.5, 1.7,      'room1');
	room1Items.push(chair1);

	books = new Model("books_glb", inventoryBooks, 
						2.5, 0.2, -0.69,
						2,
						0.4, 0.4, 0.4,
						0, 0, 0,
	...
	// textAlign(CENTER);
	// buffer2.textSize(20);
	// buffer2.textStyle(BOLD);
	// buffer2.text("Checklist", width/2, 50);
	// buffer2.textSize(12);
	// buffer2.textStyle(NORMAL);
	...
	room2container.addChild(mirror);
	...
	modernRug = new GLTF({
		asset: 'modern_rug_glb',
		x:0, y:0, z:-0.9,
		scaleX:0.65,
		scaleY:0.65,
		scaleZ:0.65
	});
	room2container.addChild(modernRug);
	...
	spots.push(new Spot(-2, 0, -1, "room2"));
	spots.push(new Spot(1.5, 0, 2, "room2"));

	//shelf spot 
	spots.push(new Spot(-2, 0.35, -1.7, "room2")); //for towel 

	//sink spot
	spots.push(new Spot(2.3, 1.1, 0.7, "room2"));//for toothbrush 
	spots.push(new Spot(2.3, 1.1, -0.4, "room2"));//for toothpaste

	//bathtub spot 
	spots.push(new Spot(-1, 0.9, -1, "room2"));//for shampoo


	//// model syntax: 
	  // new Model(asset,     x, y, z,     scale,     containerWidth, containerHeight, containerDepth,     rotateX, rotateY, rotateZ,     offsetX, offsetY, offsetZ,     heightOffset,     correctX, correctZ,     room)
	//// Room Items Push into universal items array
	toothbrush = new Model("toothbrush_glb", inventoryToothbrush,    -1, 0.4, 0,      0.5,      0.2, 0.8, 0.2,     0, 0, 0,     0, 0.2, 0,     0.3,       2.3,0.7,    'room2');
	room2Items.push(toothbrush);
	toothpaste = new Model("toothpaste_gltf", inventoryToothpaste,    -2, 2, -1.8,      0.001,       0.7, 0.2,0.3,     0, -90, 0,     0, 0, 0.0,     0,       2.3,-0.4,    'room2');
	room2Items.push(toothpaste);
	towel = new Model("towel_gltf", inventoryTowel,    1, 0.2, 1,      0.003,      1, 0.4, 1,     0, 0, 0,     0.13, 0, 0.1,     0,       -2,-1.7,    'room2');
	room2Items.push(towel);
	shampoo= new Model("shampoo_gltf", inventoryShampoo,    1.5, 1.1, 1.8,      0.04,      0.3, 0.8, 0.3,     0, -90, 30,     0.5, -0.4, 0.3,     0.46,       -1,-1,    'room2');
	...
	// textAlign(CENTER);
	// buffer3.textSize(20);
	// buffer3.textStyle(BOLD);
	// buffer3.text("Checklist", width/2, 50);
	// buffer3.textSize(14);
	// buffer3.textStyle(NORMAL);
	...
	//// model syntax: 
	  // new Model(asset,     x, y, z,     scale,     containerWidth, containerHeight, containerDepth,     rotateX, rotateY, rotateZ,     offsetX, offsetY, offsetZ,     heightOffset,     correctX, correctZ,     room)
	//// Room Items Push into universal items array
	steel_pot = new Model("pot_glb", inventoryPot,
						-2, 0.2, -1.5,		// x, y, z
						0.01,      			// scale
						0.7, 0.3, 0.4,     	// containerWidth, containerHeight, containerDepth
						0, 270, 0,     		// rotationX, Y, Z
						-.15, -.15, 0,     	// offsetX, Y, Z
						0.15,     			// heightOffset
						-0.5, -2.45,     	// correctX, Z
						'room3');
	...
	soup = new Model("soup_gltf", inventorySoup,
						1, 1.2, -2.5,		// x, y, z
						.004,      			// scale
	...
	toaster = new Model("toaster_gltf", inventoryToaster,
						-2.2, 0.8, 1.3,		// x, y, z
						5,      			// scale
	...
	paper = new Model("paper_gltf", inventoryPaper,
						-1.2, 0.2, 0.6,		// x, y, z
						0.01,      			// scale
	...
	banana = new Model("banana_gltf", inventoryBanana,
						1.5, 0.2, -2,		// x, y, z
						90,      			// scale
	...
	dish = new Model("dish_glb", inventoryDishes,
						1, 0.2, 0.5,		// x, y, z
						2.3,      			// scale
...
function draw() {
	...
	if (world.isControllerLeftTriggerDown()) {
	...
	console.log(level1);
	buttonCounter += 1;
	buttonCounter = constrain(buttonCounter, -1, 60);

	if (  keyIsDown(80)  ) {
		buttonCounter = 0;
		console.log(instructionBuffer);
	}
	if (  world.isControllerAButtonDown()  ) {
		buttonCounter = 0;
		console.log(instructionBuffer);
	}
	...
	if (instructionPage == 1) {
		instructionBuffer.clear();
		instructionBuffer.image(instruction1, 0, 0, 500, 500);
	}
	if (instructionPage == 2) {
		instructionBuffer.clear();
		instructionBuffer.image(instruction2, 0, 0, 500, 500);
	}
	if (instructionPage == 3) {
		instructionBuffer.clear();
		instructionBuffer.image(instruction3, 0, 0, 500, 500);
	}
	if (instructionPage == 4) {
		instructionBoard.hide();
	}

	//////// NOTE ////////////
	if ( keyIsDown(79) ) {
		console.log(noteBoard.getVisibility());
		noteBoard.show();
	}
	else {
		noteBoard.hide();
	...
	if ( world.isControllerBButtonDown() ) {
		noteBoard.show();
	}
	else {
		noteBoard.hide();
	...
	if (userCurrentRoom == 'room1') {
		noteBuffer.image(note1, 0, 0, 300, 300);
	}
	if (userCurrentRoom == 'room2') {
		noteBuffer.image(note2, 0, 0, 300, 300);
	}
	if (userCurrentRoom == 'room3') {
		noteBuffer.image(note3, 0, 0, 300, 300);
	}
	...
	if (level1 == "complete") {
		...
		timeUsedRoom1 = 60 - levelTime;
	}
	if (level2 == "complete") {
		timeUsedRoom2 = 60 - levelTime;
		//document.getElementById("subtitle").innerHTML = "Hit [SPACE] to proceed to Level 3";
	}
	if (level3 == "complete") {
		timeUsedRoom3 = 60 - levelTime;
	...
	if (level1 == "complete" || level2 == "complete" || level3 == "complete") {
		levelCompleteBoard.show();
		levelCompleteBuffer.clear();
		levelCompleteBuffer.image(levelCompleteImage, 0, 0, 500, 500);
		levelCompleteBuffer.textSize(40);
		levelCompleteBuffer.textStyle(BOLD);
		levelCompleteBuffer.fill(0);
		levelCompleteBuffer.text(60 - levelTime, 231, 157);


		if (60 - levelTime <= 15) {
			levelCompleteBuffer.image(ratingS, 170, 220, 160, 160);
		}
		else if (60 - levelTime <= 30) {
			levelCompleteBuffer.image(ratingA, 170, 220, 160, 160);
		}
		else {
			levelCompleteBuffer.image(ratingB, 170, 220, 160, 160);
		}
	...
	levelTime = int(levelFrame / 60);
	levelFrame = constrain(levelFrame, 0, 10000);
	if (instructionPage == 4) {
		if (level1 == 'incomplete' && level2 == 'incomplete' && level3 == 'incomplete') {
			levelFrame -= 1;
			hourGlass.spinY(3);
		}
	}
	...
	if (room1Items[0].correctStatus == "YES" && 
		room1Items[1].correctStatus == "YES" && 
		room1Items[2].correctStatus == "YES" && 
		room1Items[3].correctStatus == "YES") {
		level1 = "complete";
	}
	else {
		level1 = "incomplete";
	...
	if (room2Items[0].correctStatus == "YES" &&
		room2Items[1].correctStatus == "YES" &&
		room2Items[2].correctStatus == "YES" &&
		room2Items[3].correctStatus == "YES") {
		level2 = "complete";
	}
	else {
		level2 = "incomplete";
...
class Spot {
	...
	itemDetect() {
		for (let i = 0; i < room1Items.length; i++) {
			if (room1Items[i].container.x == this.spot.x && 
				room1Items[i].container.z == this.spot.z && 
				room1Items[i].room == this.room) {
				this.spot.setOpacity(0);
			}
		}

		for (let i = 0; i < room2Items.length; i++) {
			if (room2Items[i].container.x == this.spot.x && 
				room2Items[i].container.z == this.spot.z && 
				room2Items[i].room == this.room) {
				this.spot.setOpacity(0);
			}
		}
		...
		for (let i = 0; i < room3Items.length; i++) {
			if (room3Items[i].container.x == this.spot.x && 
				room3Items[i].container.z == this.spot.z && 
				room3Items[i].room == this.room) {
				this.spot.setOpacity(0);
			}
		}
	}
...
class Model {
	constructor(asset, inventoryAsset, 
				x, y, z, 
				scale, 
	...
	modelMove() {
		this.model.setPosition( this.container.x + this.offsetX, 
								this.container.y + this.offsetY, 
								this.container.z + this.offsetZ )
	}
	...
	inventoryImage() {
		if (this.container.y == 10) {
			inventoryBoard.show();
			inventoryBuffer.image(this.inventoryAsset, 0, 0, 255, 255);
		}
	}
}
...
class Sensor {
	...
	limit() {
		if (userCurrentRoom == "room1") {
			if (this.x > 2.5) {
				world.setUserPosition(2.5, userHeight, this.z);
			}
			if (this.x < -2.5) {
				world.setUserPosition(-2.5, userHeight, this.z);
			}
			if (this.z > 2.5) {
				world.setUserPosition(this.x, userHeight, 2.5);
			}
			if (this.z < -2.5) {
				world.setUserPosition(this.x, userHeight, -2.5);
			}
		...
		if (userCurrentRoom == "room2") {
			if (this.x > 12.5) {
				world.setUserPosition(12.5, userHeight, this.z);
			}
			if (this.x < 7.5) {
				world.setUserPosition(7.5, userHeight, this.z);
			}
			if (this.z > 12.5) {
				world.setUserPosition(this.x, userHeight, 12.5);
			}
			if (this.z < 7.5) {
				world.setUserPosition(this.x, userHeight, 7.5);
			}
		}

		if (userCurrentRoom == "room3") {
			if (this.x > 22.5) {
				world.setUserPosition(22.5, userHeight, this.z);
			}
			if (this.x < 17.5) {
				world.setUserPosition(17.5, userHeight, this.z);
			}
			if (this.z > 22.5) {
				world.setUserPosition(this.x, userHeight, 22.5);
			}
			if (this.z < 17.5) {
				world.setUserPosition(this.x, userHeight, 17.5);
			}
		}
	}
}
...
function goToLevel2() {
	...
	world.setUserPosition(10, userHeight + 0.2, 10);
	setTimeout(function(){
...
function goToLevel3() {
	userCurrentRoom = "room0";
	world.setUserPosition(20, userHeight, 20);
	setTimeout(function(){
